...
def test_remix_step() -> None:
	...
	assert steps[1].get('args') == args_2
        ...
        assert re.match(r'target-1_source-1_\d{8}-\d{6}\.jpg', steps[2].get('args').get('target_path'))
        ...
        assert re.match(r'target-1_source-1_\d{8}-\d{6}\.jpg', steps[3].get('args').get('target_path'))
